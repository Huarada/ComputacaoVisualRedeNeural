...
// Fragment shader com suporte a textura
// (usada só no plano de entrada texturizado, camada conv extra)
...
// Carrega textura 2D de arquivo (path lido do argv[3])
...
public:
    ...
    bool LoadInputAndBuildLayout(int argc,char** argv){
        ...
        // Camada convolucional extra da imagem de entrada (sempre no índice 0)
        InjectTextureInputStage();
...
private:
    ...
    // Insere camada convolucional extra (imagem de entrada) na posição 0
    void InjectTextureInputStage(){
        int imgA = 0, imgB = 0;
        bool found = false;

        // Tenta usar o tamanho da primeira camada conv real encontrada
        ...
        Branch imageBranch;
        LayerUnit imageUnit;
        imageUnit.kind        = LayerUnit::kConvolution;
        imageUnit.neuronCount = 0;
        imageUnit.A           = imgA;
        imageUnit.B           = imgB;
        imageUnit.C           = 1; // exatamente 1 folha
        imageBranch.push_back(imageUnit);
        imageStage.push_back(std::move(imageBranch));

        parsedStages.insert(parsedStages.begin(), std::move(imageStage));
    }
    ...
    void BuildVisualLayersAndComputeBounds(){
        ...
        // Offsets especiais para a camada de imagem (stage 0):
        // mais à esquerda e ligeiramente mais alta que o pipeline principal.
        const float kTextureStageOffsetX = -4.0f; // empurra a imagem para a esquerda
        const float kTextureStageOffsetY =  5.0f; // eleva a imagem acima da linha das camadas
        ...
        for(int s=0; s<(int)parsedStages.size(); ++s){
            ...
            // Posição base em X do estágio
            float stageBaseX    = stageX[s];
            float stageOffsetX  = 0.0f;
            float stageOffsetY  = 0.0f;

            // Para o estágio 0 (imagem de entrada), desloca à esquerda e para cima
            if (s == 0) {
                stageOffsetX = kTextureStageOffsetX;
                stageOffsetY = kTextureStageOffsetY;
            }
            ...
            for(size_t b=0; b<stage.size(); ++b){
                ...
                if(branchHasConv){
                    ...
                    for(size_t i=0;i<branch.size();++i){
                        ...
                        Vec3 center {
                            stageBaseX + stageOffsetX,
                            yStart + i*kRowSpacingY + stageOffsetY,
...
public:
    ...
    bool TryLoadColors(const std::string& filepath, CnnModelLayout& layout) {
        ...
        if (loadedNeuronColors.size() < (size_t)layout.numberNeurons)
            return false;
...
private:
    ...
    // Aplica a textura SOMENTE na folha da camada conv extra de entrada
    void DrawConvolutionLayerVisuals(const std::vector<ConvLayerVisual>& visuals){
        ...
        // visuals[0] é a camada conv extra de imagem
        ...
        const int targetPanelGlobalIndex  = panelsInFirstBlock - 1; // como C=1, índice 0
